...
export class CaloriePanel extends HTMLElement {
    constructor() {
        super();
        this.db = new DbEngine();
        this.currentView = 'daily';
        this.paramValues = {};
        this.SKILL_NAME = 'calorie';
    ...
    render() {
      ...
      <div class="panel">
        ...
        <div class="views-nav" id="views-nav" style="display:none">
          ${config.views
            .filter(v => v.components.table || v.components.chart || v.components.dataBrowser)
            .map(v => `
    ...
    async loadDb(file) {
        try {
            await this.db.loadFromFile(file, 'calorie_data.db');
    ...
    async switchView() {
        ...
        if (!view)
            return;
        try {
            // 优先渲染 dashboard（多个视图用 dashboard 做入口）
            if (view.id === 'daily' || view.id === 'weight' || view.id === 'exercise' || view.id === 'sleep') {
                await renderDashboardView(this, this.db);
                // 附带渲染表格（如果 view 有 table 组件）
  ...
  <button class="retry-btn" onclick="this.closest('calorie-panel').reload()">重试</button>
</div>`;
        }
    }
...
customElements.define('calorie-panel', CaloriePanel);
