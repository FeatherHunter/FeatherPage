...
export async function renderTableView(panel, db, queryId, tableConfig) {
    ...
    const schemaFields = config.schema.tables.flatMap(t => t.fields);
    // 合并 schema 字段和查询定义的字段（后者覆盖前者，用于计算列如 gap）
    const allFields = query.fields
        ? [...schemaFields, ...query.fields.filter(qf => !schemaFields.some(sf => sf.name === qf.name))]
        : schemaFields;
    // 渲染表格
