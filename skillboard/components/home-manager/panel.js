...
export class HomeManagerPanel extends HTMLElement {
    constructor() {
        super();
    ...
    async switchView() {
        ...
        try {
            ...
            else if (view.components.form) {
                await this.renderFormView(view.components.form.actionId);
            }
  ...
  <button class="retry-btn" onclick="this.closest('home-manager-panel').reload()">重试</button>
</div>`;
        }
    }
    ...
    async renderFormView(actionId) {
        const content = this.shadow.getElementById('view-content');
        const action = config.actions.find(a => a.id === actionId);
        if (!action) {
            content.innerHTML = `<div class="empty-state"><p>Action not found: ${actionId}</p></div>`;
            return;
        }
        const fields = action.fields.map(f => {
            const isRequired = f.required ? '<span class="required">*</span>' : '';
            const opts = f.options
                ? `<select name="${f.field}" id="field-${f.field}">${f.options.map(o => `<option value="${o}">${o}</option>`).join('')}</select>`
                : `<input type="text" name="${f.field}" id="field-${f.field}" placeholder="${f.prompt || f.field}" />`;
            return `<div class="form-field">
        <label for="field-${f.field}">${f.prompt || f.field}${isRequired}</label>
        ${opts}
      </div>`;
        }).join('');
        content.innerHTML = `
      <form class="item-form" id="item-form">
        <h3 class="form-title">${action.label}</h3>
    ...
    `;
        const form = this.shadow.getElementById('item-form');
        ...
        form?.addEventListener('submit', async (e) => {
            ...
            try {
                await this.db.exec(this.buildInsertSql(action), values);
    ...
    buildInsertSql(action) {
        const table = action.targetTable;
        const fields = action.fields.filter(f => f.source === 'user-input');
        const colNames = fields.map(f => f.field).join(', ');
