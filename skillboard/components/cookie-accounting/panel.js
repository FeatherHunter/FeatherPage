...
import { renderCompareView } from './renderers/compare.js';
import { renderTimeDimensionView } from './renderers/time-dimension.js';
import { resolveShortcut, getMonthLastDay } from '../../shared/utils.js';
export class CookieAccountingPanel extends HTMLElement {
    ...
    constructor() {
        ...
        this.paramValues = {};
        this.SKILL_NAME = 'cookie-accounting';
        this.shadow = this.attachShadow({ mode: 'open' });
    }
    async connectedCallback() {
        this.render();
        // Listen for db-updated from Settings page (loads via settings panel)
        this.addEventListener('db-updated', async (e) => {
            ...
            if (restored) {
                ...
                // Restore failed: show warning but keep drag prompt visible
                this.shadow.querySelector('.db-status')?.remove();
        ...
        if (savedFilename) {
            // Desktop Chrome: use File System Access API for auto-reload (fresh data from file)
            // @ts-ignore
            ...
            // Fallback: restore from localStorage, then prompt user to reselect file for fresh data
            const restored = await this.restoreDbFromStorage();
            if (restored) {
    ...
    bindEvents() {
        ...
        this.shadow.querySelectorAll('.nav-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                this.currentView = btn.getAttribute('data-view') || 'dashboard';
    ...
    async loadDb(file) {
        try {
            await this.db.loadFromFile(file, 'biscuit_accountant.db');
            ...
            // Persist file info for auto-reload on next page load
    ...
    async persistFileHandle(file) {
        // File System Access API: 暂时禁用 picker，存储文件名用于下次尝试自动刷新
    ...
    `;
        ...
        this.shadow.getElementById('db-reload-btn')?.addEventListener('click', async () => {
            const reloaded = await this.tryAutoReload();
            if (reloaded) {
                this.switchView(); // 重新渲染当前视图以显示最新数据
            }
            // 用户取消 picker: 不做任何处理，保持现有数据
        });
    }
    ...
    async switchView() {
        ...
        try {
            ...
            else if (view.id === 'compare') {
                const mode = this.paramValues['_compare_mode'] || 'month';
                await renderCompareView(this, this.db, mode);
            }
            else if (view.id === 'time-dimension') {
                await renderTimeDimensionView(this, this.db);
            }
  ...
  <button class="retry-btn" onclick="this.closest('cookie-accounting-panel').reload()">重试</button>
</div>`;
        }
    }
    async prefillParams(queryId) {
        ...
        // Auto-calculate month_lastday when month is set
        if (this.paramValues['month']) {
            this.paramValues['month_lastday'] = getMonthLastDay(this.paramValues['month']);
        }
    }
    // ── 暴露给渲染器的快捷访问 ──────────────────────
...
customElements.define('cookie-accounting-panel', CookieAccountingPanel);
