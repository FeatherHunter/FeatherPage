...
/**
 * 渲染通用图表视图
 * 支持：月度统计(bar)、收支总览(?)、分类分析(doughnut)、周期对比(bar)
 */
export async function renderChartView(panel, db, queryId) {
    ...
    content.innerHTML = `${paramHtml}<div class="chart-wrapper${chartType === 'doughnut' ? ' doughnut-wrapper' : ''}"><canvas id="chart-canvas"></canvas></div>`;
